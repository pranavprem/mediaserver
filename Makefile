...
portainer-up:
	@echo "🚀 Starting standalone Portainer bootstrap stack..."
	$(PORTAINER_COMPOSE) up -d
	@echo "✅ Portainer started."

portainer-down:
	@echo "🛑 Stopping standalone Portainer bootstrap stack..."
...
portainer-logs:
	$(PORTAINER_COMPOSE) logs --tail=200 -f
...
# ─── Seerr Migration / Update ───────────────────────────────────────────────

# Migrate/update the request UI from Jellyseerr to Seerr using the existing data path
update-seerr:
	@echo "🎬 Updating Jellyseerr service to Seerr image..."
	@test -n "$(CONFIG_ROOT)" && [ "$(CONFIG_ROOT)" != "/path/to/config" ] || (echo "❌ Set CONFIG_ROOT in .env first." && exit 1)
	@test -d "$(CONFIG_ROOT)/jellyseerr" || (echo "❌ Missing $(CONFIG_ROOT)/jellyseerr. Start Jellyseerr/Seerr once or check CONFIG_ROOT." && exit 1)
	@mkdir -p "$(JELLYSEERR_BACKUP_ROOT)"
	@backup="$(JELLYSEERR_BACKUP_ROOT)/jellyseerr-$$(date +%Y%m%d-%H%M%S).tar.gz"; \
		echo "💾 Backing up Jellyseerr config to $$backup"; \
		tar -C "$(CONFIG_ROOT)" -czf "$$backup" jellyseerr
	@echo "🔐 Ensuring config is owned by Seerr's node user (1000:1000)..."
	docker run --rm -v "$(CONFIG_ROOT)/jellyseerr:/data" alpine chown -R 1000:1000 /data
	@echo "🔄 Pulling Seerr image..."
	docker compose pull jellyseerr
	@echo "🚀 Recreating request UI service..."
	docker compose up -d jellyseerr
	@echo "✅ Seerr update complete. Check logs with: docker compose logs --tail=100 jellyseerr"
...
help:
	...
	@echo "  portainer-up         - Start standalone Portainer"
	@echo "  portainer-down       - Stop standalone Portainer"
	@echo "  portainer-restart    - Restart standalone Portainer"
	@echo "  portainer-logs       - Tail standalone Portainer logs"
	@echo "  portainer-ps         - Show standalone Portainer status"
	@echo "  update-seerr         - Backup Jellyseerr config and recreate it with the Seerr image"
