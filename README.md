...
- Request UI with **Seerr** driving Sonarr/Radarr (service/container kept as `jellyseerr` for compatibility)
...
- **Portainer** as a standalone bootstrap stack for central stack management
- **Watchtower** for automatic container updates (gluetun and Immich excluded — see below)
...
| `media` | Media streaming & management | jellyfin, jellyseerr (Seerr), plex, recyclarr |
| `proxy` | Cloudflare tunnel access | cloudflared, jellyfin, jellyseerr (Seerr), plex, vaultwarden, immich-server, paperless-webserver |
...
| `management` | Git + auto-updates | watchtower, gitea |
...
- `portainer-compose.yaml` — standalone Portainer bootstrap stack
- `stack-ops-compose.yaml` — pasteable Portainer one-shot job for clean restarts
...
| `make portainer-up` | Starts standalone Portainer using `portainer-compose.yaml` |
| `make portainer-down` | Stops standalone Portainer |
| `make portainer-restart` | Restarts standalone Portainer |
| `make portainer-logs` | Tails standalone Portainer logs |
| `make portainer-ps` | Shows standalone Portainer status |
| `make update-seerr` | Backs up Jellyseerr config, fixes Seerr ownership, pulls the maintained Seerr image, and recreates the request UI |
...
**Portainer bootstrap:** Portainer is intentionally not part of the main `mediaserver` stack. Start it with `make portainer-up`, then import this repo into Portainer as a Git-backed stack named `mediaserver`.

**Portainer stack migration:** see `docs/portainer-stack-migration.md`.

**Clean restart all stacks from Portainer:** create a Portainer stack from `stack-ops-compose.yaml`, then start the exited `stack-clean-restart` container when needed. See `docs/portainer-clean-restart-snippet.md`.
...
This stack keeps the Docker service/container name and config path as `jellyseerr` so existing Cloudflare tunnel routes, monitoring, and automation shortcuts continue to work while the image runs Seerr.

Before the first Seerr start after migrating from Jellyseerr, run the migration/update target:

```bash
make update-seerr
```

Disable "Tag Requests" in Seerr's Radarr settings if requests aren't arriving.
...
**Database image:** Immich Postgres uses the Immich-maintained `ghcr.io/immich-app/postgres` image with VectorChord/pgvector extensions. Keep the existing `${CONFIG_ROOT}/immich/postgres` volume mounted; do not recreate it during upgrades.

**Updates:** Immich services are excluded from Watchtower because `immich-server` and the database extension image need coordinated upgrades. Upgrade Immich manually from Portainer or compose, then verify `immich-postgres` before restarting `immich-server`.
...
### Prometheus — Metrics
`http://NAS_IP:9090` — scrapes cAdvisor (container metrics), node-exporter (host metrics), and itself. Additional compose projects can be added as scrape targets (see below).
...
To scrape metrics from containers in other compose projects, connect their network-parent container to the `monitoring` network:
